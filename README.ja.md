...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 006 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 006 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 006 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 006 作例 8"></td>
  </tr>
</table>

## 3:4 上下構成作例

16:9 組とは別の独立した4点で、3:4 上下の完成キャンバスを生成。上部は現実の写真を保ち、下部は Panel 006 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 006 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 006 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 006 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 006 追加上下作例 4"></td>
  </tr>
</table>
...
> 主体 10〜20% · 紙の余白 80〜90% · 細い手描き線 · 4色以内 · アクリル平塗り

XXD Panel 006 は、Codex と互換 Agent のための画像生成 Skill です。写真にある本当の身元、動き、機能、感情を保ちながら、主体を画面の約 10〜20% に収まる小さく集中した挿絵へ徹底的に整理します。残り 80〜90% は、粗い白紙や淡い紙の能動的な余白です。
...
写真をポスター、表紙、SNS コンテンツ、壁紙として使うとき、課題は単に画風を変えることではありません。再設計した後も、主体の同一性、重要な関係、空気感、見分けやすさを保つことが大切です。

**Panel 006** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真を、小さな主体と大きな紙の余白を持つアートブック表紙へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。

### このような場面に適しています

- **内容を保ちたい：** 人物、物、動き、関係を無関係なテンプレートに置き換えたくない。
- **表現を強めたい：** 写真に明確な編集デザイン、芸術言語、物語の焦点が必要。
- **柔軟に展開したい：** 同じ素材から比較構成、デザインのみ、複数比率、壁紙一式を作りたい。

### 簡単な始め方

最初からすべての設定を読む必要はありません。画像を Agent に渡し、次のように伝えてください。

> XXD Panel 006 でこの画像を処理して、まず最適な構図とサイズを提案して。

視覚方向を先に確認したい場合は、上の作例をご覧ください。準備ができたら[使い始める](#使い始める)へ進めます。全モードと引数は折りたたみ資料から必要に応じて確認できます。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 006 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、小さな主体と大きな紙の余白を持つアートブック表紙へ |
| **ひと目で分かる特徴** | 主体 10〜20% · 紙の余白 80〜90% · 細い手描き線 · 4色以内 · アクリル平塗り |
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-006` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-006-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-006-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 006 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-006 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-006 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 006 を適用し、共通設定は一度だけ解決
/xxd-panel-006 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
- 選択した通常モードごとに1点を返し、`wallpaper-pack` を選ぶと独立壁紙4点を追加します。`全部` は元写真1枚につき7点を同じ新規タスクフォルダ直下へ出力し、一覧コラージュやモード別サブフォルダは作りません。
...
この表は歴代 001–060 Panel のディレクトリです。各項目は独立した原文プロンプトと美的ロジックを保持し、現在のプロジェクトを太字で示します。
