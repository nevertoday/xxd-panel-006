...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 006 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 006 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 006 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 006 예시 8"></td>
  </tr>
</table>

## 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 006의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 006 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 006 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 006 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 006 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 주제 10–20% · 종이 여백 80–90% · 가는 손선 · 최대 네 색 · 아크릴 평면

XXD Panel 006는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진 속 실제 정체성, 동작, 기능과 감정을 보존하면서 주제를 디자인 화면의 약 10–20%를 차지하는 작고 집중된 삽화로 과감히 줄입니다. 나머지 80–90%는 거친 흰 종이나 밝은 종이의 능동적인 여백입니다.
...
**Panel 006**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 작은 주제와 넓은 종이 여백의 아트북 표지로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.

### 이런 상황에 적합합니다

- **내용을 보존해야 할 때:** 사람, 사물, 동작과 관계를 무관한 템플릿으로 바꾸고 싶지 않습니다.
- **표현을 강화해야 할 때:** 사진에 더 분명한 편집 디자인, 예술 언어 또는 서사적 초점이 필요합니다.
- **다양하게 전달해야 할 때:** 같은 소재로 비교 구성, 디자인 전용 결과, 여러 비율 또는 배경화면 세트가 필요합니다.

### 간단한 시작 방법

처음부터 모든 옵션을 읽을 필요는 없습니다. Agent에 이미지를 보내고 다음과 같이 요청하세요.

> XXD Panel 006으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.

시각 방향을 먼저 확인하려면 위 예시를 살펴보세요. 준비가 되면 [시작하기](#시작하기)로 이동할 수 있습니다. 전체 모드와 매개변수는 접이식 참고 자료에서 필요할 때 확인할 수 있습니다.
<!-- xxd-human-intro:end -->
...
## 빠른 판단: XXD Panel 006이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 작은 주제와 넓은 종이 여백의 아트북 표지로 |
| **한눈에 알아볼 특징** | 주제 10–20% · 종이 여백 80–90% · 가는 손선 · 최대 네 색 · 아크릴 평면 |
...
```text
원본 사실 고정 → 주제를 10–20%의 작은 삽화로 축소 → 80–90%의 거친 종이를 주요 공간으로 사용 → 가는 손선으로 구조 제시 → 소수의 완전한 아크릴 면으로 주제 구축 → 원본 색을 네 가지 이하로 압축
```

무관한 사진으로 바꿔도 작은 삽화의 윤곽, 핵심 색면, 여백 관계와 문구가 거의 그대로 성립한다면 006가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **작은 주제, 높은 식별성:** 삽화는 약 10–20%지만 최소 세 가지 원본 고유 단서로 정체성, 동작, 기능과 감정을 보존합니다.
- **여백이 공간:** 80–90%의 거친 흰 종이, 미색 종이, 밝은 종이가 규모감, 고요함과 출판물의 성격을 맡습니다.
- **절제된 비대칭:** 주제는 위·아래·옆·가장자리에 놓일 수 있지만 자세, 동작, 읽기 방향과 문구에 응답해야 합니다.
- **가늘고 불안정한 선:** 구조, 경계, 연결, 질감과 식별만 알리며 순수 선화나 떨리는 노이즈가 되지 않습니다.
- **완전한 아크릴 평면:** 소수의 불투명한 면이 주제를 만들고 물감 침착, 종이결과 약간 불규칙한 가장자리를 남깁니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 006의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-006 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-006 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-006 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-006 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 006 적용, 공통 설정은 한 번만 확정
/xxd-panel-006 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-006`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-006-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-006-prompt.en.md)
...
- 선택한 일반 모드마다 파일 1개를 반환하고, `wallpaper-pack`을 선택하면 독립 배경화면 4개를 추가합니다. `전체`는 원본 한 장당 PNG 7개를 하나의 새 작업 폴더 바로 아래에 저장하며 모음 이미지나 모드별 하위 폴더를 만들지 않습니다.
...
**주제는 알아볼 수 있을 만큼만 작게, 중요함은 종이의 여백이 만들게 합니다.**
