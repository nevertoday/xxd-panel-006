...
## Four outputs, one paper-cover logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 006 small-subject paper-cover illustration below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 006 small-subject paper-cover illustration right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `006.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different vignette positions, paper ratios, four-colour selections, environmental hints, and type counterweights.
...
## Copy must counterbalance the tiny subject through whitespace
...
Automatic copy distils one title, theme word, object name, place, or short phrase from subject identity, visible action, function, relation, known place, or a supported quiet meaning. It should feel inseparable from the subject rather than use vague poetry to simulate sophistication.
...
The image model creates the 10–20% vignette, 80–90% paper field, fine hand line, complete acrylic blocks, four-colour source palette, and book-cover type counterweight. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-006-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-006-prompt.en.md)
