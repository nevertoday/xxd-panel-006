...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-paper-cover-logic)
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2089997569829093688"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 006 sample 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2089997569829093688"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 006 sample 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2089997569829093688"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 006 sample 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2089997569829093688"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 006 sample 4"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    ...
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 006 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 006 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 006 Sample 8"></td>
  </tr>
</table>

## Additional 3:4 Top–Bottom Samples

These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 006's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 006 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 006 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 006 additional top-bottom sample 3"></td>
...
**Panel 006** is suited to work that must retain the source's essential content while pursuing this visual direction: **Reduce a photograph to a tiny recognisable subject on a vast tactile paper cover**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 006 on this image and recommend the best layout and size first.
...
006 reverses that logic:

```text
lock source facts → reduce to a 10–20% vignette → make 80–90% rough paper the primary space → indicate structure with fine hand line → build the subject with a few complete acrylic blocks → compress source colour to four or fewer
```
...
- **Restrained asymmetry:** the vignette may sit high, low, to one side, or near an edge, but placement must answer pose, action, reading direction, and copy.
- **Fine unstable line:** line only indicates structure, boundary, connection, texture, and recognition; it never becomes pure line art or nervous noise.
- **Complete flat acrylic:** a few opaque blocks establish the subject, preserving painted deposits, paper grain, and gently irregular edges.
- **Four colours maximum:** beyond paper, no more than four source-derived principal colours appear, each with an identity or structural role.
- **Minimal setting:** at most a few short lines or one restrained colour plane; never reconstruct the full scene or fill the paper.
- **Book-cover type:** one concise title and at most two small lines counterbalance the vignette through whitespace.
...
```text
/xxd-panel-006 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-006 photo.jpg --prefs last
/xxd-panel-006 photo.jpg --prefs edit
/xxd-panel-006 photo.jpg --prefs new
```
...
/xxd-panel-006 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-006 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-006 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-006 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-006 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-006 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-006 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-006 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-006 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-006 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 006 to a whole directory; shared settings resolve once
/xxd-panel-006 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-006 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
npx skills add https://github.com/nevertoday/xxd-panel-006 --skill xxd-panel-006
```
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-006`. Restart the agent session after installation.
...
- [Chinese runtime adapter](references/xxd-panel-006-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-006-prompt.en.md)
...
<p align="center">
  <a href="https://xiaoxiaodong.pages.dev/assets/wechat-qr.png"><img src="https://xiaoxiaodong.pages.dev/assets/wechat-qr.png" alt="XXD purchase and contact WeChat QR code" width="320"></a>
</p>

<div align="center">

**Make the subject only large enough to recognise; let the paper make it matter.**
